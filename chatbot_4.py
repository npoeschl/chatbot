...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not (contract_dbqueries.isValidUser(context._user_id)):
        ...
        reply_markup = InlineKeyboardMarkup(keyboard)
        if (update.message):
            await update.message.reply_text("Ich bin dein Vertrags-Knecht. \xF0\x9F\x9A\x98 Was kann ich für dich tun?", reply_markup=reply_markup)
        else:
            query = update.callback_query
            await query.answer()
            await query.edit_message_text("Ich bin dein Vertrags-Knecht. \xF0\x9F\x9A\x98 Was kann ich für dich tun?", reply_markup=reply_markup)
